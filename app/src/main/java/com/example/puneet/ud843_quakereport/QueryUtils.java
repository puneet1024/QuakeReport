...
public final class QueryUtils {
    ...
    public static ArrayList<Earthquake> extractEarthquakes() {
        ...
        try {
            ...
            //Get the instance of JSONArray that contains the features JSONobjects
            JSONArray jsonArray = jsonRootObject.optJSONArray("features");

            //Iterate through each of feature array
            for(int i=0;i<jsonArray.length();i++) {
                ...
                JSONObject propJsonObject = jsonObject.optJSONObject("properties");

                //Extract "mag" for magnitude
                double mag = propJsonObject.optDouble("mag");
                //Extract "place" for place
                String place = propJsonObject.optString("place");
                //Extract "time" for time
                long time = propJsonObject.optLong("time");

                String url = propJsonObject.optString("url");


                //Use the instance of SimpleDateFormat to convert it into useable format
//                SimpleDateFormat dateFormat = new SimpleDateFormat("MMM DD, yyyy HH:MM:SS");
//                String dateToDisplay = dateFormat.format(new Date(time));
//                Log.v("millis2","changing time in millisec2");


                Earthquake earthquake = new Earthquake(mag,place,time,url);
                earthquakes.add(earthquake);
                Log.v("Added","earthquake object added successfully");
